...
- The control props pattern and how we can let the user of our hook or component
  be in control of it's state from the outside by letting them pass their own
  state as props
- How to show useful warnings for when the user might use our hook incorrectly
  by e.g. changing it from being controlled to uncontrolled or vice versa or by
  passing a state prop without an onChange prop
