...
- The prop collections and prop getters patterns, how they can be used and how
  they differ
- Prop collection has our custom hook return an object with props, a collection,
  that should be used (like aria stuff) which helps users from forgetting to set
  those props
- Prop getters are functions returned from our custom hook that the user can
  call and specify which props they need or if they need something else to also
  happen in event handlers e.g, onClick
