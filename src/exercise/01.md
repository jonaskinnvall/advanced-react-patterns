...
- The Context module functions pattern and how to implement it
- How the pattern can help users by encapsulating complex state changes in a
  utility function, that takes care of those state updates, which the user can
  call with the dispatch function and all else that is needed as arguments
- By exporting this utility function from the same module as the context
  provider (and other relevant functions) this function can be tree-shaken and
  lazy loaded
