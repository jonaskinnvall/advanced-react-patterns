...
- The state reducer pattern, inversion of control and how it is utilized in this
  pattern to enable the user to be in control of state changes in our custom
  hook
  - Done by letting the user pass a reducer of their own to handle state changes
  - We can help the user even further by both:
    - Defaulting the reducer prop to our reducer so user doesn't have to pass a
      reducer of their own
    - But also by exporting our reducer so they can utilize it and just make
      changes to specific action types they want to control and otherwise return
      our default reducer they've imported
