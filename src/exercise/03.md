...
- How to make the compound components pattern more flexible
  - It's done by using context to manage the state between the components
    instead of passing it through with React.Children.map and React.cloneElement
  - Allows the user to render the compound components as grand children of
    Toggle
- The compound components pattern should usually be used in this way, althoough
  there are usecases for using React.Children.map and React.cloneElement if
  e.g., it is important that the compound components are rendered as direct
  children
